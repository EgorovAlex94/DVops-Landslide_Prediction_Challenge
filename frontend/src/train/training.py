"""
Программа: Тренировка модели на backend, отображение метрик и графиков на экране
"""
import optuna
import os
import json
...
def start_training(config: dict, endpoint: object) -> None:
    ...
    """
    # Last metrics
    if os.path.exists(config["train"]["metric_path"]):
        with open(config["train"]["metric_path"]) as json_file:
            old_metrics = json.load(json_file)
    else:
        # если до этого не обучали модель
